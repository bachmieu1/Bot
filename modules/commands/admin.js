...
module.exports.run = async function ({ api, event, args, Users, permssion }) {
    ...
    switch (args[0]) {
        ...
        case "add": {
            if (permssion != 2) return api.sendMessage("[Admin] mày đéo đủ tuổi để có thể sử dụng chức năng 'add'", threadID, messageID);
        ...
        case "rm":
        case "delete": {
            if (permssion != 2) return api.sendMessage("[Admin] Mày đéo đủ tuổi để có thể sử dụng chức năng 'delete'", threadID, messageID);
